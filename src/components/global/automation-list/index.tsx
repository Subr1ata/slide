...
const AutomationList = (props: Props) => {
  ...
  const optimisticUiData = useMemo(() => {
    if (latestVariable && latestVariable?.variables && data) {
      ...
      const uniqueTest = Array.from(
        test
          .reduce((map, item) => {
            if (!map.has(item.id)) {
              map.set(item.id, item); // Store unique items in the map
            }
            return map;
          }, new Map())
          .values()
      ...
      return { data: uniqueTest as any };
      // return { data: test };
    }
    return data || { data: [] };
  }, [latestVariable, data]);

  console.log("optimisticUiData.data ---> ", optimisticUiData.data);
            ...
            {automation.keywords.length > 0 ? (
              <div className="flex gap-x-2 flex-wrap mt-3">
                {automation.keywords.map(
                  (keyword: { id: string; word: string }, key: number) => (
                    <div
                      key={keyword.id}
                      className={cn(
